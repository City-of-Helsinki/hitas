...
def test_calculate__single_improvement__housing_company__pre_2011__market_price_index__after_2010():
    ...
    result = calculate_single_housing_company_improvement_pre_2011_market_price_index(
        ...
        housing_company_shares_count=1,  # Irrelevant in this test
        apartment_shares_count=1,  # Irrelevant in this test
    )
...
def test_calculate__single_improvement__housing_company__pre_2011__market_price_index__before_2010__no_deductions():
    improvement = ImprovementData(
        name="Test improvement",
        value=Decimal(150_000),
        completion_date=datetime.date(2009, 5, 1),
    ...
    result = calculate_single_housing_company_improvement_pre_2011_market_price_index(
        ...
        total_surface_area=Decimal(100.0),
        apartment_surface_area=Decimal(20.0),
        housing_company_shares_count=100,
        apartment_shares_count=20,
    )
    ...
    assert result.depreciation.time.years == 0
    assert result.depreciation.time.months == 0
    assert roundup(result.accepted_value_for_housing_company) == 180_000
    assert roundup(result.accepted_value) == 36_000


def test_calculate__multiple_improvements__housing_company__pre_2011__market_price_index__before_2010__no_deductions():
    improvement = ImprovementData(
        name="Test improvement",
        value=Decimal(150_000),
        completion_date=datetime.date(2009, 5, 1),
    ...
    assert result.items[0].depreciation.time.years == 0
    assert result.items[0].depreciation.time.months == 0
    assert roundup(result.items[0].accepted_value_for_housing_company) == 180_000
    assert roundup(result.items[0].accepted_value) == 36_000
    ...
    assert result.summary.excess.value_per_square_meter_before_2010 == 150
    assert result.summary.excess.value_per_square_meter_after_2010 is None
    assert result.summary.excess.total_before_2010 == 15000
    assert result.summary.excess.total_after_2010 is None
    assert result.summary.value_without_excess == 360_000
    assert result.summary.depreciation == 0
    assert result.summary.accepted_value_for_housing_company == 360_000
    assert result.summary.accepted_value == 72_000
